...
(define tests
  (test-suite
   ...
   (check-equal? (sequence 0 6 2) (list 0 2 4 6) "Sequence test")
   (check-equal? (sequence 0 0 2) (list 0) "Sequence test")
   ...
   (check-equal? (stream-for-n-steps (stream-add-zero funny-number-stream) 3)
   ...
   ; vector-assoc test
   (check-equal? (vector-assoc 3 (vector (cons 2 1) (cons 4 1))) #f "vector-assoc test")
   (check-equal? (vector-assoc 3 (vector (cons 2 1) (cons 4 1) 3)) #f "vector-assoc test")
   (check-equal? (vector-assoc 3 (vector (cons 2 1) (cons 3 1) (cons 4 1))) (cons 3 1) "vector-assoc test")
   (check-equal? (vector-assoc 3 (vector (cons 2 1) (cons 3 1) (cons 3 2))) (cons 3 1) "vector-assoc test")
   ...
   ; while-less test
   ; (define a 2) is at the top of the file
   (check-equal? (while-less 7 do (begin (set! a (+ a 1)) a)) #t "while-less test")
   (check-equal? a 7 "while-less test")
